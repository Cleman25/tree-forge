...
export const NextAppDetector = {
    ...
    generate: async (n, cfg) => {
        ...
        if (existingFiles.length > 0 && !cfg.overwriteMode) {
            throw new Error(`Cannot create Next.js app in "${n.path}" - directory contains existing files:\n` +
...
export const FirebaseDetector = {
    ...
    generate: async (n, cfg) => {
        ...
        if (existingFiles.length > 0 && !cfg.overwriteMode) {
            throw new Error(`Cannot initialize Firebase in "${n.path}" - directory contains existing files:\n` +
                existingFiles.map(f => `  - ${f}`).join('\n') + '\n\n' +
                'Options:\n' +
                '1. Remove existing files\n' +
                '2. Use --overwriteMode=force to overwrite\n' +
        ...
        try {
            ...
            throw new Error('Firebase CLI tools not found. Please install first:\n' +
...
export const PkgDetector = {
    ...
    generate: async (n, cfg) => {
        ...
        if (existingFiles.length > 0 && !cfg.overwriteMode) {
            throw new Error(`Cannot initialize package.json in "${n.path}" - file already exists\n\n` +
                'Options:\n' +
        ...
        try {
            ...
            throw new Error(`Package manager "${pm.init[0]}" not found. Please install it first.`);
        }
        actions.push({
            type: "exec",
            cwd: n.path,

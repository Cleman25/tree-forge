...
import type { ForgeConfig, LogLevel, PlanAction, TreeNode, DetectorEvents } from "./types.js";
...
const argv = await yargs(hideBin(process.argv))
  ...
  .option("pm", {
    type: "string",
    choices: ["pnpm", "yarn", "npm"] as const,
  ...
  .option("log-level", { 
    type: "string", 
    choices: ["silent", "error", "warn", "info", "debug", "verbose"] as const,
    default: "info",
    describe: "Log level" 
  })
  .option("log-format", { 
  ...
  try {
    ...
    if (argv.list) {
      ...
      if (recentLogs.length > 0) {
        ...
        recentLogs.forEach((file: string, i: number) => {
      ...
      if (recentStats.length > 0) {
        console.log(chalk.blue("\nRecent stats files:"));
        recentStats.forEach((file: string, i: number) => {
          console.log(chalk.dim(`  ${i + 1}. ${file}`));
        });
      }
...
const cfg: ForgeConfig = {
  ...
  treeText: argv.tree as string | undefined,
  treeFile: argv["tree-file"] as string | undefined,
  dryRun: !!argv["dry-run"],
  yes: !!argv.yes,
  packageManager: (argv.pm as any) || undefined,
  ...
  logging: {
    ...DEFAULT_LOG_CONFIG,
    enabled: argv.log,
    level: argv["log-level"] as LogLevel,
    format: argv["log-format"] as "text" | "json",
    console: !argv.quiet,
...
if (!treeSource) {
  ...
  console.error(
    chalk.red(
      "No ASCII tree provided. Use --tree '<ascii>' or --tree-file path/to/tree.txt (lines like 'apps/\\n  web/\\n    package.json')."
    )
  );
  process.exit(1);
}
...
const plan: PlanAction[] = planFromTree(roots, cfg);
...
if (cfg.runDetectors) {
  ...
  const matches: { detectorId: string; node: TreeNode }[] = [];
  ...
  if (!cfg.quiet) {
    detectorManager.on('init:start', ({ detectorId, node }: DetectorEvents['init:start']) => {
      console.log(chalk.blue(`\n  ⚡ Running ${detectorId} detector...`));
      console.log(chalk.dim(`    🔍 Scanning ${node.path}...`));
    });

    detectorManager.on('init:validate', ({ detectorId, node, isValid }: DetectorEvents['init:validate']) => {
      if (!isValid) {
        console.log(chalk.yellow(`    ⚠ Validation failed for ${node.path}`));
      }
    });

    detectorManager.on('init:prompt', ({ detectorId, node, confirmed }: DetectorEvents['init:prompt']) => {
      if (!confirmed) {
        console.log(chalk.yellow(`    ↪ Skipped by user choice`));
      }
    });

    detectorManager.on('init:generate', ({ detectorId, node, actions }: DetectorEvents['init:generate']) => {
      console.log(chalk.green(`    ✓ Generated ${actions.length} actions`));
    });

    detectorManager.on('init:complete', ({ detectorId, node, success }: DetectorEvents['init:complete']) => {
      if (success) {
        console.log(chalk.green(`    ✨ Successfully initialized ${node.path}`));
      }
    });

    detectorManager.on('init:error', ({ detectorId, node, error }: DetectorEvents['init:error']) => {
  ...
  for (const d of detectorList) {
    if (!chosenIds.includes(d.id)) continue;
    for (const r of roots) collectMatches(r, d, matches, cfg);
  }
...
if (!cfg.quiet) console.log(chalk.blue(`📦 Ready to execute ${plan.length} actions...`));
const ok = await confirmPlan(plan.length, cfg.yes);
if (!ok) {
  if (!cfg.quiet) console.log(chalk.yellow('⏹ Operation cancelled by user'));
  process.exit(0);
}

if (!cfg.quiet) console.log(chalk.blue('🚀 Executing actions...'));
const logger = new Logger(cfg.logging, cfg.targetDir);
await applyPlan(plan, cfg.dryRun, logger);
if (!cfg.quiet) console.log(chalk.green('✨ All actions completed successfully'));

// compute repoRoot the same way generators do
const repoRoot =
  roots.length === 1 && roots[0].kind === "dir" ? roots[0].path : cfg.targetDir;
...
function walkMatch(n: TreeNode, match: (n: TreeNode, cfg: ForgeConfig) => boolean, cfg: ForgeConfig): boolean {
  if (match(n, cfg)) return true;
  return n.children.some((c) => walkMatch(c, match, cfg));
}

function collectMatches(n: TreeNode, d: ReturnType<typeof getDetectors>[number], out: { detectorId: string; node: TreeNode }[], cfg: ForgeConfig) {
  if (d.match(n, cfg)) out.push({ detectorId: d.id, node: n });
  for (const c of n.children) collectMatches(c, d, out, cfg);
}
...
function countPlanItems(plan: PlanAction[]): { dirs: number; files: number } {
  const result = { dirs: 0, files: 0 };
  for (const action of plan) {
    if (action.type === 'mkdir') result.dirs++;
    if (action.type === 'write') result.files++;
  }
  return result;
}
...
interface FileStats {
  ...
  emptyDirs: number;
  largestDir: { path: string; fileCount: number };
  configFiles: number;
  sourceFiles: number;
  testFiles: number;
  dotFiles: number;
...
function analyzeTreeStructure(nodes: TreeNode[]): { 
  totalDirs: number;
  totalFiles: number;
  rootDirs: TreeNode[];
  rootFiles: TreeNode[];
  ...
  function traverse(node: TreeNode, depth: number = 0) {
    ...
    if (node.kind === 'dir') {
      ...
      node.children.forEach(child => {
        if (child.kind === 'file') dirFileCount++;
      });

      if (dirFileCount === 0) emptyDirs++;
      ...
      // Check patterns
      if (patterns.config.test(node.name)) stats.configFiles++;
      if (patterns.source.test(node.name)) stats.sourceFiles++;
      if (patterns.test.test(node.name)) stats.testFiles++;
      if (patterns.dot.test(node.name)) stats.dotFiles++;
      if (patterns.documentation.test(node.name)) stats.documentationFiles++;
      if (patterns.style.test(node.name)) stats.styleFiles++;
      if (patterns.asset.test(node.name)) stats.assetFiles++;
      if (patterns.package.test(node.name)) stats.packageFiles++;
      if (patterns.lockfile.test(node.name)) stats.lockFiles++;
      if (patterns.ci.test(node.name)) stats.ciFiles++;

...
if (cfg.runDetectors) {
  ...
  for (const d of detectors) {
    ...
    for (const r of roots) collectMatches(r, d, matches);
  }
...
function walkMatch(n: TreeNode, match: (n: TreeNode) => boolean): boolean {
  if (match(n)) return true;
  return n.children.some((c) => walkMatch(c, match));
}

function collectMatches(n: TreeNode, d: typeof detectors[number], out: { detectorId: string; node: TreeNode }[]) {
  if (d.match(n)) out.push({ detectorId: d.id, node: n });
  for (const c of n.children) collectMatches(c, d, out);
}

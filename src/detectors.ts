import type { CustomInitializer, Detector, ForgeConfig, GeneratorResult, TreeNode } from "./types.js";
...
export const CustomDetector = (init: CustomInitializer): Detector => ({
  id: init.id,
  match: (n, cfg) => {
    ...
    if (init.matchFiles) {
      const hasMatchingFiles = n.children.some(child => 
        init.matchFiles!.includes(child.name)
      );
      // If matching files exist, only match if we're allowed to overwrite
      if (hasMatchingFiles && !cfg.overwriteMode) return false;
    }
...
const ServiceDetector = createGenericDetector(
  "service-init",
...
const LibraryDetector = createGenericDetector(
  "library-init",
  ["lib", "package", "sdk", "utils", "core", "shared", "common"],
  {
    dirs: ["src", "test", "docs"],
    packageJson: {
      scripts: {
...
const ConfigDetector = createGenericDetector(
  "config-init",
  ["config", "settings"],
  {
...
const InfraDetector = createGenericDetector(
  "infra-init",
  ["infra", "deploy", "ops"],
  {
...
const DatabaseDetector = createGenericDetector(
  "db-init",
...
const UIDetector = createGenericDetector(
  "ui-init",
...
const DocsDetector = createGenericDetector(
  "docs-init",
...
export const getDetectors = (cfg: ForgeConfig): Detector[] => {
  const builtIn = [
    NextAppDetector,
    FirebaseDetector,
    PkgDetector,
